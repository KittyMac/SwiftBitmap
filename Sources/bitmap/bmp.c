...
Bitmap *bm_load_mem(const void *buffer, long len) {
    SET_ERROR("no error");
...
static int bm_save_bmp(Bitmap *b, FILE *f);
static int bm_save_gif(Bitmap *b, FILE *f);
static int bm_save_pcx(Bitmap *b, FILE *f);
static int bm_save_tga(Bitmap *b, FILE *f);
#ifdef USEPNG
static int bm_save_png(Bitmap *b, FILE *f);
#endif
#ifdef USEJPG
static int bm_save_jpg(Bitmap *b, FILE *f);
#endif
...
#endif
    if(gif)
        return bm_save_gif(b, f);
    if(pcx)
        return bm_save_pcx(b, f);
    if(tga)
        return bm_save_tga(b, f);
    return bm_save_bmp(b, f);
}

int bm_convert(Bitmap *b, const char *fname, char ** memory_buffer, size_t * len) {
    SET_ERROR("no error");
    /* Chooses the file type to save as based on the
    extension in the filename */
    char *lname = strdup(fname), *c,
    ...
    f = open_memstream (memory_buffer, len);
